	...
	module.exports = {
	    ...
	    getPolls: (req, res) => {
	        db_poll.Polls.findAll({
	                ...
	                attributes: ['question', 'userId', 'id'],
	                include: [{
	                    model: db_poll.Poll_Options,
	                    attributes: [
	                        ['id', 'optionId'], 'text', [sequelize.fn('COUNT', sequelize.col('poll_options.votes.id')), 'voteCount']
	                    ],
	                    include: {
	                        model: db_poll.Votes,
	                        attributes: []
	    ...
	    deletePoll: (req, res) => {
	        db_poll.Polls.destroy({
	                where: { id: req.query.id }
	            })
	            .then(() => {
	                return db_poll.Poll_Options.destroy({
	                    where: { pollId: null }
	                })
	            })
	            .then(() => {
	                return db_poll.Votes.destroy({
	                    where: { pollOptionId: null }
	                })
	            })
